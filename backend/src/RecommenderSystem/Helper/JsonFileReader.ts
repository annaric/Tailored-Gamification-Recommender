...
export default class JsonFileReader {
    ...
    checkValidityOfJsonFile(literatureObject: LiteratureElementObject[]): boolean {
      ...
      literatureObject.forEach(element => {
        ...
        if (! (element.resultType in LiteratureResultTypeEnum)) {
          throw new Error("Invalid Json File: expected 'resultType' keyInvalid Json File: expected 'resultType' key to be one of the LiteratureResultTypeEnum values");
        }
